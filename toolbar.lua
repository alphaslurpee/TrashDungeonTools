...
function TDT:initToolbar(frame)
    db = TDT:GetDB()

    frame.toolbar = CreateFrame("Frame","TDTToolbarFrame",frame)
    ...
    frame.toolbar.tex:SetColorTexture(unpack(TDT.BackdropColor))
    ...
    frame.toolbar.toggleButton:SetNormalTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\arrows")
    frame.toolbar.toggleButton:GetNormalTexture():SetTexCoord(0,1,0.65,1)

    frame.toolbar:Hide()
    frame.toolbar:SetScript("OnHide",function()
        TDT:UpdateSelectedToolbarTool(nil)
    end)
    ...
    TDT:FixAceGUIShowHide(frame.toolbar.widgetGroup,frame.toolbar)
    ...
    local widgets = {}
    TDT.tempWidgets = widgets

    ---back
    local back = AceGUI:Create("Icon")
    back:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.5,0.75,0.5,0.75)
    ...
    forward:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.75,1,0.5,0.75)
    ...
    minus:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0,0.25,0.5,0.75)
    ...
    plus:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.25,0.5,0.5,0.75)
    ...
    pencil:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0,0.25,0,0.25)
    toolbarTools["pencil"] = pencil
    pencil:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "pencil" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("pencil") end
    end)
    ...
    line:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0,0.25,0.75,1)
    toolbarTools["line"] = line
    line:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "line" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("line") end
    end)
    ...
    arrow:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.25,0.5,0,0.25)
    toolbarTools["arrow"] = arrow
    arrow:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "arrow" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("arrow") end
    end)
    ...
    note:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.75,1,0,0.25)
    toolbarTools["note"] = note
    note:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "note" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("note") end
    end)
    ...
    mover:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.5,0.75,0,0.25)
    toolbarTools["mover"] = mover
    mover:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "mover" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("mover") end
    end)
    ...
    cogwheel:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0,0.25,0.25,0.5)
    cogwheel:SetCallback("OnClick",function (widget,callbackName)
        InterfaceOptionsFrame_OpenToCategory("TrashDungeonTools")
        InterfaceOptionsFrame_OpenToCategory("TrashDungeonTools")
        TDT:HideInterface()
    end)
    ...
    eraser:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.25,0.5,0.25,0.5)
    toolbarTools["eraser"] = eraser
    eraser:SetCallback("OnClick",function (widget,callbackName)
        if currentTool == "eraser" then TDT:UpdateSelectedToolbarTool() else TDT:UpdateSelectedToolbarTool("eraser") end
    end)
    ...
    delete:SetImage("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons",0.25,0.5,0.75,1)
    ...
    for k,widget in ipairs(widgets) do
        ...
        widget:SetCallback("OnEnter",function(widget,callbackName)
            TDT:ToggleToolbarTooltip(true,widget)
        end)
        widget:SetCallback("OnLeave",function()
            TDT:ToggleToolbarTooltip(false)
        end)
    ...
    TDT:CreateBrushPreview(frame)
    TDT:UpdateSelectedToolbarTool()
...
local function getTexture()
    ...
    if size == 0 then
        return TDT.main_frame.mapPanelFrame:CreateTexture(nil, "OVERLAY")
    else
...
function TDT:ReleaseAllActiveTextures()
...
---CreateBrushPreview
function TDT:CreateBrushPreview(frame)
    frame.brushPreview = CreateFrame("Frame","TrashDungeonToolsBrushPreview",UIParent)
    ...
    frame.brushPreview.tex:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\ring")
    frame.brushPreview.tex:SetAllPoints()
end

---EnableBrushPreview
function TDT:EnableBrushPreview(tool)
    ...
    frame.brushPreview:SetScript("OnUpdate", function(self, tick)
        if MouseIsOver(TDTScrollFrame) and not MouseIsOver(TDTToolbarFrame) then
...
---DisableBrushPreview
function TDT:DisableBrushPreview()
    local frame = TDT.main_frame
...
function TDT:ToggleToolbarTooltip(show, widget)
    if not show then
        GameTooltip:Hide()
    else
        local yOffset = -1
...
function TDT:UpdateSelectedToolbarTool(widgetName)
    local toolbar = TDT.main_frame.toolbar
    if not widgetName or (not toolbarTools[widgetName]) then
        if toolbar.highlight then toolbar.highlight:Hide() end
        TDT:RestoreScrollframeScripts()
        TDT:DisableBrushPreview()
        if drawingActive then
            if currentTool == "pencil" then TDT:StopPencilDrawing() end
            if currentTool == "arrow" then TDT:StopArrowDrawing() end
            if currentTool == "line" then TDT:StopLineDrawing() end
            if currentTool == "mover" then TDT:StopMovingDrawing() end
            if currentTool == "eraser" then TDT:StopEraserDrawing() end
        end
        currentTool = nil
    ...
    toolbar.highlight:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\icons")
    ...
    TDT:OverrideScrollframeScripts()
    TDT:EnableBrushPreview(currentTool)
...
function TDT:OverrideScrollframeScripts()
    local frame = TDT.main_frame
    frame.scrollFrame:SetScript("OnMouseDown", function(self,button)
        if button == "LeftButton" then
            if currentTool == "pencil" then TDT:StartPencilDrawing() end
            if currentTool == "arrow" then TDT:StartArrowDrawing() end
            if currentTool == "line" then TDT:StartLineDrawing() end
            if currentTool == "mover" then TDT:StartMovingObject() end
            if currentTool == "eraser" then TDT:StartEraserDrawing() end
        end
    ...
    frame.scrollFrame:SetScript("OnMouseUp", function(self,button)
        if button == "LeftButton" then
            ...
            if currentTool == "arrow" then TDT:StopArrowDrawing() end
            if currentTool == "line" then TDT:StopLineDrawing() end
            if currentTool == "mover" then TDT:StopMovingObject() end
            if currentTool == "eraser" then TDT:StopEraserDrawing() end
            if currentTool == "note" then TDT:StartNoteDrawing() end
        end
        if button == "RightButton" then
            local scrollFrame = TDT.main_frame.scrollFrame
        ...
        if currentTool == "mover" then
            for note,_ in pairs(notePoolCollection.pools.QuestPinTemplate.activeObjects) do
                ...
                note:SetScript("OnDragStop", function()
                    ...
                    if TDT.liveSessionActive then TDT:LiveSession_SendNoteCommand("move",note.objectIndex,x-xOffset,y-yOffset) end
...
function TDT:RestoreScrollframeScripts()
    local frame = TDT.main_frame
...
function TDT:GetCursorPosition()
    local frame = TDT.main_frame
    local scrollFrame = frame.scrollFrame
    local relativeFrame = UIParent      --UIParent
    local mapPanelFrame = TDT.main_frame.mapPanelFrame
...
function TDT:GetHighestFrameLevelAtCursor()
...
local nobj
---StartArrowDrawing
function TDT:StartArrowDrawing()
    drawingActive = true
    local frame = TDT.main_frame
    local startx,starty = TDT:GetCursorPosition()
    local line = getTexture()
    line:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\Square_White")
    line:SetVertexColor(db.toolbar.color.r,db.toolbar.color.g,db.toolbar.color.b,db.toolbar.color.a)
    line:Show()
    local arrow = getTexture()
    arrow:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\triangle")
    ...
    ---l: x1,y1,x2,y2,...
    ---t: triangleroation
    nobj = { d={ db.toolbar.brushSize, 1, TDT:GetCurrentSubLevel(), true, TDT:RGBToHex(db.toolbar.color.r,db.toolbar.color.g,db.toolbar.color.b)}, l={}}
    nobj.l = { TDT:Round(startx,1), TDT:Round(starty,1)}
    nobj.t = {}
    local scale = TDT:GetScale()
    frame.toolbar:SetScript("OnUpdate", function(self, tick)
        if not MouseIsOver(TDTScrollFrame) then return end
        local x,y = TDT:GetCursorPosition()local currentDrawLayer = TDT:GetHighestFrameLevelAtCursor()
        drawLayer = max(drawLayer,currentDrawLayer)
        if x~= startx and y~=starty then
            DrawLine(line, TDT.main_frame.mapPanelTile1, startx, starty, x, y, (db.toolbar.brushSize*0.3)*scale, 1,"TOPLEFT")
            nobj.l[3] = TDT:Round(x,1)
            nobj.l[4] = TDT:Round(y,1)
        end
        --position arrow head
        arrow:Show()
        ...
        arrow:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x,y)
...
---StopArrowDrawing
function TDT:StopArrowDrawing()
    local frame = TDT.main_frame
    TDT:StorePresetObject(nobj)
...
---StartLineDrawing
function TDT:StartLineDrawing()
    drawingActive = true
    local frame = TDT.main_frame
    startx,starty = TDT:GetCursorPosition()
    local line = getTexture()
    line:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\Square_White")
    line:SetVertexColor(db.toolbar.color.r,db.toolbar.color.g,db.toolbar.color.b,db.toolbar.color.a)
    line.isOwn = true
    ...
    ---d: size,lineFactor,sublevel,shown,colorstring,drawLayer,[smooth]
    ---l: x1,y1,x2,y2,...
    nobj = { d={ db.toolbar.brushSize, 1.1, TDT:GetCurrentSubLevel(), true, TDT:RGBToHex(db.toolbar.color.r,db.toolbar.color.g,db.toolbar.color.b), nil, true}, l={}}
    nobj.l = {}

    local scale = TDT:GetScale()
    frame.toolbar:SetScript("OnUpdate", function(self, tick)
        if not MouseIsOver(TDTScrollFrame) then return end
        local currentDrawLayer = TDT:GetHighestFrameLevelAtCursor()
        drawLayer = max(drawLayer,currentDrawLayer)
        endx,endy = TDT:GetCursorPosition()
        if endx~= startx and endy~=starty then
            DrawLine(line, TDT.main_frame.mapPanelTile1, startx, starty, endx, endy, (db.toolbar.brushSize*0.3)*1.1*scale, 1.00,"TOPLEFT")
            line:SetDrawLayer(objectDrawLayer,drawLayer)
            line:Show()
            TDT:DrawCircle(startx,starty,(db.toolbar.brushSize*0.3)*scale,db.toolbar.color,objectDrawLayer,drawLayer,true,nil,circle1,true)
            TDT:DrawCircle(endx,endy,(db.toolbar.brushSize*0.3)*scale,db.toolbar.color,objectDrawLayer,drawLayer,true,nil,circle2,true)
            nobj.d[6] = drawLayer
        end
    end)
end
...
function TDT:StopLineDrawing()
    ...
    for i=1,numSegments do
        ...
        nobj.l[4*i-3] = TDT:Round(x,1)
        nobj.l[4*i-2] = TDT:Round(y,1)
        nobj.l[4*i-1] = TDT:Round(newx,1)
        nobj.l[4*i] = TDT:Round(newy,1)
        x,y = newx,newy
    end
    tinsert(nobj.l, TDT:Round(x,1))
    tinsert(nobj.l, TDT:Round(y,1))
    tinsert(nobj.l, TDT:Round(endx,1))
    tinsert(nobj.l, TDT:Round(endy,1))
...
function TDT:StartPencilDrawing()
    drawingActive = true
    local frame = TDT.main_frame
    oldx = nil
    oldy = nil
    ...
    ---d: size,lineFactor,sublevel,shown,colorstring,drawLayer,[smooth]
    ---l: x1,y1,x2,y2,...
    nobj = { d={ db.toolbar.brushSize, 1.1, TDT:GetCurrentSubLevel(), true, TDT:RGBToHex(db.toolbar.color.r,db.toolbar.color.g,db.toolbar.color.b), 0, true}, l={}}
    nobj.l = {}
    ...
    frame.toolbar:SetScript("OnUpdate", function(self, tick)
        ...
        local x,y = TDT:GetCursorPosition()
        local mapScale = TDT.main_frame.mapPanelFrame:GetScale()
        ...
        if (oldx and abs(x-oldx)>threshold) or (oldy and abs(y-oldy)>threshold)  then
            TDT:DrawLine(oldx,oldy,x,y,(db.toolbar.brushSize*0.3)*scale,db.toolbar.color,true,objectDrawLayer,layerSublevel,nil,true)
            nobj.d[6] = layerSublevel
            nobj.l[lineIdx] = TDT:Round(oldx,1)
            nobj.l[lineIdx+1] = TDT:Round(oldy,1)
            nobj.l[lineIdx+2] = TDT:Round(x,1)
            nobj.l[lineIdx+3] = TDT:Round(y,1)
            lineIdx = lineIdx + 4
            oldx,oldy = x,y
        end
    end)
...
function TDT:StopPencilDrawing()
    local frame = TDT.main_frame
    local x,y = TDT:GetCursorPosition()
    local layerSublevel = TDT:GetHighestFrameLevelAtCursor()
    local scale = TDT:GetScale()
    --finish line
    if x~=oldx or y~=oldy then
        TDT:DrawLine(oldx,oldy,x,y,(db.toolbar.brushSize*0.3)*scale,db.toolbar.color,true,objectDrawLayer,layerSublevel)
        --store it
        local size = 0
        for k,v in ipairs(nobj.l) do
            size = size+1
        end
        nobj.l[size+1] = TDT:Round(oldx,1)
        nobj.l[size+2] = TDT:Round(oldy,1)
        nobj.l[size+3] = TDT:Round(x,1)
        nobj.l[size+4] = TDT:Round(y,1)
    end
    frame.toolbar:SetScript("OnUpdate",nil)
    --clear own flags
    for k,v in pairs(activeTextures) do
    ...
    if lineCount > 0 then
        ...
        TDT:DrawCircle(x,y,db.toolbar.brushSize*0.3*scale,db.toolbar.color,objectDrawLayer,layerSublevel)
        TDT:StorePresetObject(nobj)
...
function TDT:StartMovingObject()
    --we have to redraw all objects first, as the objectIndex needs to be set on every texture
    TDT:DrawAllPresetObjects()
    drawingActive = true
    local frame = TDT.main_frame
    objectIndex = TDT:GetHighestPresetObjectIndexAtCursor()
    local startx,starty = TDT:GetCursorPosition()
    originalX,originalY = TDT:GetCursorPosition()
    frame.toolbar:SetScript("OnUpdate", function(self, tick)
        if not MouseIsOver(TDTScrollFrame) then return end
        local x,y = TDT:GetCursorPosition()
...
function TDT:HideAllPresetObjects()
    --drawings
...
---StopMovingDrawing
function TDT:StopMovingObject()
    local frame = TDT.main_frame
    frame.toolbar:SetScript("OnUpdate",nil)
    if objectIndex then
        local newX,newY = TDT:GetCursorPosition()
        TDT:UpdatePresetObjectOffsets(objectIndex,originalX-newX,originalY-newY)
...
function TDT:GetHighestPresetObjectIndexAtCursor()
    local currentSublevel = -8
...
function TDT:StartEraserDrawing()
    TDT:DrawAllPresetObjects()
    drawingActive = true
    local frame = TDT.main_frame
    local startx,starty
    ...
    frame.toolbar:SetScript("OnUpdate", function(self, tick)
        ...
        local x,y = TDT:GetCursorPosition()
        if x~=startx or y ~=starty then
            local highestObjectIdx = TDT:GetHighestPresetObjectIndexAtCursor()
            for j,tex in pairs(activeTextures) do
...
---StopEraserDrawing
function TDT:StopEraserDrawing()
    local frame = TDT.main_frame
    frame.toolbar:SetScript("OnUpdate",nil)
...
function TDT:StartNoteDrawing()
    ...
    local x,y = TDT:GetCursorPosition()
    nobj = {d={ x, y, TDT:GetCurrentSubLevel(), true, ""}}
    nobj.n = true
    TDT:StorePresetObject(nobj)
    if self.liveSessionActive then self:LiveSession_SendObject(nobj) end
    TDT:DrawAllPresetObjects()
...
---DrawCircle
function TDT:DrawCircle(x, y, size, color, layer, layerSublevel, isOwn, objectIndex, tex, noinsert, extrax, extray)
    local circle = tex or getTexture()
    if not layer then layer = objectDrawLayer end
    circle:SetDrawLayer(layer, layerSublevel)
    circle:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\Circle_White")
    ...
    circle:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x,y)
    circle:Show()
    circle.isOwn = isOwn
...
---DrawLine
function TDT:DrawLine(x, y, a, b, size, color, smooth, layer, layerSublevel, lineFactor, isOwn, objectIndex)
    local line = getTexture()
    if not layer then layer = objectDrawLayer end
    line:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\Square_White")
    line:SetVertexColor(color.r,color.g,color.b,color.a)
    DrawLine(line, TDT.main_frame.mapPanelTile1, x, y, a, b, size, lineFactor and lineFactor or 1.1,"TOPLEFT")
    ...
    tinsert(activeTextures,line)
    if smooth == true  then
        TDT:DrawCircle(x,y,size,color,layer,layerSublevel,isOwn,objectIndex)
    end
end

---DrawTriangle
function TDT:DrawTriangle(x, y, rotation, size, color, layer, layerSublevel, isOwn, objectIndex)
    local triangle = getTexture()
    if not layer then layer = objectDrawLayer end
    triangle:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\triangle")
    ...
    triangle:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x,y)
...
local function updateNoteObjText(text,note)
    ...
    if TDT.liveSessionActive then TDT:LiveSession_SendNoteCommand("text",note.objectIndex,text) end
end
local function deleteNoteObj(note)
    local currentPreset = TDT:GetCurrentPreset()
    tremove(currentPreset.objects,note.objectIndex)
    if TDT.liveSessionActive then TDT:LiveSession_SendNoteCommand("delete",note.objectIndex,"0") end
    TDT:DrawAllPresetObjects()
end
...
local function makeNoteEditbox()
    ...
    TDT:FixAceGUIShowHide(editbox,nil,nil,true)
    editbox.frame:SetScript("OnShow",function()
        hooksecurefunc(TDT, "MouseDownHook", function() editbox.frame:Hide() end)
        hooksecurefunc(TDT, "ZoomMap", function() editbox.frame:Hide() end)
    end)
...
---DrawNote
function TDT:DrawNote(x, y, text, objectIndex)
    if not notePoolCollection then
        notePoolCollection = CreateFramePoolCollection()
        notePoolCollection:CreatePool("Button", TDT.main_frame.mapPanelFrame, "QuestPinTemplate")
    end
    local scale = TDT:GetScale()
    --setup
    ...
    note:ClearAllPoints()
    note:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x,y)
