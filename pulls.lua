...
function TDT:ReleaseHullTextures()
...
function TDT:DrawHullCircle(x, y, size, color, layer, layerSublevel)
...
function TDT:DrawHullLine(x, y, a, b, size, color, smooth, layer, layerSublevel, lineFactor)
    ...
    if smooth == true  then
        TDT:DrawHullCircle(x,y,size,color,layer,layerSublevel)
    end
end

function TDT:DrawHull(vertices,pullColor)

    local hull = convex_hull(vertices)
    if hull and hull[#hull] and #hull>2 then

        hull = expand_polygon(hull,7*(TDT.scaleMultiplier[TDT:GetDB().currentDungeonIdx] or 1))
        --hull = smooth_contour(hull,10)
        for i = 1, #hull do
            local a = hull[i]
            local b = hull[1]
            if i ~= #hull then b = hull[i+1] end
            TDT:DrawHullLine(a[1], a[2], b[1], b[2], 3*(TDT.scaleMultiplier[TDT:GetDB().currentDungeonIdx] or 1), pullColor, true, "ARTWORK", -8, 1)
        end
    end
end
    ...
    for enemyIdx,clones in pairs(p) do
        if tonumber(enemyIdx) then
            for _,cloneIdx in pairs(clones) do
                if TDT:IsCloneIncluded(enemyIdx, cloneIdx) then
...
function TDT:DrawAllHulls(pulls)
    TDT:ReleaseHullTextures()
    ...
    for pullIdx,p in pairs(pulls) do
        local r,g,b = TDT:DungeonEnemies_GetPullColor(pullIdx,pulls)
        vertices = getPullVertices(p,blips)
        TDT:DrawHull(vertices,{r=r, g=g, b=b, a=1})
    end
end

function TDT:FindClosestPull(x,y)
    local preset = TDT:GetCurrentPreset()
    local blips = TDT:GetDungeonEnemyBlips()
