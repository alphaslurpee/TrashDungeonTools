...
function TDT:POI_CreateFramePools()
    TDT.poi_framePools = TDT.poi_framePools or CreateFramePoolCollection()
    TDT.poi_framePools:CreatePool("Button", TDT.main_frame.mapPanelFrame, "MapLinkPinTemplate")
    TDT.poi_framePools:CreatePool("Button", TDT.main_frame.mapPanelFrame, "DeathReleasePinTemplate")
    TDT.poi_framePools:CreatePool("Button", TDT.main_frame.mapPanelFrame, "VignettePinTemplate")
    TDT.poi_framePools:CreatePool("Frame", TDT.main_frame.mapPanelFrame, "TDTAnimatedLineTemplate")
end
...
local function POI_SetDevOptions(frame,poi)
    ...
    frame:SetScript("OnMouseDown", function(self, button)
        ...
        if button == "RightButton" then
            local pois = TDT.mapPOIs[db.currentDungeonIdx][TDT:GetCurrentSubLevel()]
...
local function POI_SetOptions(frame,type,poi,homeSublevel)
    ...
    if type == "mapLink" then
        ...
        frame:SetScript("OnClick",function()
            TDT:SetCurrentSubLevel(poi.target)
            TDT:UpdateMap()
            TDT:ZoomMapToDefault()
        end)
        frame:SetScript("OnEnter",function()
            GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
            GameTooltip:AddLine(TDT:GetDungeonSublevels()[db.currentDungeonIdx][poi.target], 1, 1, 1, 1)
    ...
    if type == "nyalothaSpire" then
        ...
        frame:SetScript("OnMouseUp",function(self,button)
            if button == "RightButton" then
                --reset npc location
                TDT:GetRiftOffsets()[self.npcId]=nil
                TDT:UpdateMap()
                if TDT.liveSessionActive and TDT:GetCurrentPreset().uid == TDT.livePresetUID then
                    TDT:LiveSession_SendCorruptedPositions(TDT:GetCurrentPreset().value.riftOffsets)
                end
            end
            if button == "LeftButton" then
                local _, connections = TDT:FindConnectedDoor(frame.npcId,1)
                if connections then
                    TDT:SetCurrentSubLevel(connections[#connections].target)
                    TDT:UpdateMap()
                end
            end
        end)
        ...
        frame:SetScript("OnEnter",function()
            ...
            local blips = TDT:GetDungeonEnemyBlips()
                ...
                for _,blip in pairs(blips) do
                    if blip.data.id == poi.npcId then
                        ...
                        if isBlipSameWeek then
                            ...
                            frame.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,frame,blipFrame,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
            ...
            local connectedDoor, connections = TDT:FindConnectedDoor(frame.npcId,1)
            if connectedDoor then
                if frame.isSpire then
                    frame.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,frame,connectedDoor,nil,nil,nil,nil,nil,not frame.isSpire,frame.animatedLine)
                end
                local sublevelName = TDT:GetSublevelName(nil,connections[#connections].target)
                local npcName = TDT:GetNPCNameById(frame.npcId)
        ...
        frame:SetScript("OnLeave",function()
            ...
            if frame.isSpire then
                TDT:HideAnimatedLine(frame.animatedLine)
            end
        end)
        --check expanded status
        if TDT:IsNPCInPulls(poi) then
...
function TDT:POI_HideAllPoints()
...
---POI_UpdateAll
function TDT:POI_UpdateAll()
    twipe(points)
    db = TDT:GetDB()
    local framePools = TDT.poi_framePools
    ...
    if not TDT.mapPOIs[db.currentDungeonIdx] then return end
    local currentSublevel = TDT:GetCurrentSubLevel()
    local pois = TDT.mapPOIs[db.currentDungeonIdx][currentSublevel]
    if not pois then return end
    local preset = TDT:GetCurrentPreset()
    local teeming = TDT:IsPresetTeeming(preset)
    local scale = TDT:GetScale()
    local week = TDT:GetEffectivePresetWeek(preset)
    for poiIdx,poi in pairs(pois) do
        ...
        then
            ...
            poiFrame:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",poi.x*scale,poi.y*scale)
...
local function animateLine(self, elapsed)
    ...
    for i=1,numLines + 1 do
        ...
        tex:SetPoint("TOPLEFT", TDT.main_frame.mapPanelTile1,"TOPLEFT",tX - (self.sizeX / 2),tY - (self.sizeY / 2))
        tex:SetPoint("BOTTOMRIGHT", TDT.main_frame.mapPanelTile1,"TOPLEFT",tX + (self.sizeX / 2),tY + (self.sizeY / 2))
...
local function createAnimatedLine(parent)
    local animatedLine = TDT.poi_framePools:Acquire("TDTAnimatedLineTemplate")
    animatedLine:Show()
...
function TDT:ShowAnimatedLine(parent, frame1, frame2, sizeX, sizeY, gap, color, speed, selected, animatedLine)
    if not (frame1 and frame2 and (not frame1:IsForbidden()) and (not frame1:IsForbidden())) then
        return nil
    end
    texturePool = texturePool or CreateTexturePool(TDT.main_frame.mapPanelFrame ,"OVERLAY",7,nil)
    ...
    local scale = TDT:GetScale()
    local mapSizex,mapSizey = TDT:GetDefaultMapPanelSize()
...
function TDT:KillAllAnimatedLines()
    local linePool = self.poi_framePools:GetPool("TDTAnimatedLineTemplate")
...
function TDT:DrawAllAnimatedLines()
    local week = self:GetEffectivePresetWeek()
    for _,blip in pairs(TDT:GetDungeonEnemyBlips()) do
        if not blip:IsShown() and blip.data.corrupted then
            TDT:HideAnimatedLine(blip.animatedLine)
        elseif blip.data.corrupted and blip.selected then
            ...
            local connectedDoor = TDT:FindConnectedDoor(blip.data.id)
            connectedFrame = connectedDoor or connectedFrame
            blip.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,connectedFrame,blip,nil,nil,nil,nil,nil,blip.selected)
    ...
    local _,activeSpires = TDT.poi_framePools:GetPool("VignettePinTemplate"):EnumerateActive()
    for poiFrame,_ in pairs(activeSpires) do
        if poiFrame.spireIndex and poiFrame.npcId and not poiFrame.isSpire and not poiFrame.animatedLine then
            local connectedDoor = TDT:FindConnectedDoor(poiFrame.npcId,1)
            if connectedDoor then
                poiFrame.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,poiFrame,connectedDoor,nil,nil,nil,nil,nil,not poiFrame.isSpire)
            end
        end
    end
end

function TDT:HideAnimatedLine(animatedLine)
...
function TDT:FindConnectedDoor(npcId, numConnection)
