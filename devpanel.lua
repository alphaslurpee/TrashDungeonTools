...
3. Call TDT:AddNPCFromUnit("mouseover") with macro to add untargetable units
...
function TDT:AddNPCFromUnit(unit)
    db = TDT:GetDB()
    local npcId
    ...
    local added
    for _,npcData in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    if npcId and not added then
        ...
        local npcScale = 1
        local npcCount = 0
        tinsert(TDT.dungeonEnemies[db.currentDungeonIdx], {
            name = npcName,
            health = npcHealth,
            level = npcLevel,
...
function TDT:CreateDevPanel(frame)
    db = TDT:GetDB()
    ...
    TDT:FixAceGUIShowHide(devPanel)
    ...
    local function DrawGroup1(container)
        ...
        local buttons = {
            [1] = {
                text="MapLink",
                func=function()
                    if not TDT.mapPOIs[db.currentDungeonIdx] then TDT.mapPOIs[db.currentDungeonIdx] = {} end
                    if not TDT.mapPOIs[db.currentDungeonIdx][TDT:GetCurrentSubLevel()] then
                        TDT.mapPOIs[db.currentDungeonIdx][TDT:GetCurrentSubLevel()] = {}
                    end
                    local links = TDT.mapPOIs[db.currentDungeonIdx][TDT:GetCurrentSubLevel()]
            ...
            [7] = {
                text="Export to LUA",
                func=function()
                    local export = tshow(TDT.mapPOIs[db.currentDungeonIdx],"TDT.mapPOIs[dungeonIndex]")
    ...
    local function DrawGroup2(container)
        ...
        local function updateFields(health,level,creatureType,id,scale,count,idx)
            if idx then
                local data = TDT.dungeonEnemies[db.currentDungeonIdx][idx]
        ...
        local function updateDropdown(npcId,idx)
            if not TDT.dungeonEnemies[db.currentDungeonIdx] then return end
            idx = idx or 1
            local enemies = {}
            for mobIdx,data in ipairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
        ...
        dropdown:SetCallback("OnValueChanged", function(widget,callbackName,key)
            ...
            local dungeonEnemyBlips = TDT:GetDungeonEnemyBlips()
        ...
        countSlider:SetCallback("OnMouseUp",function(widget,callbackName,value)
            ...
            local data = TDT.dungeonEnemies[db.currentDungeonIdx][npcIdx]
            data["count"] = value
            TDT:UpdateMap()
        end)
        ...
        button1:SetCallback("OnClick",function()
            local npcId = TDT:AddNPCFromUnit("target")
        ...
        button2:SetCallback("OnClick",function()
            local currentBlip = TDT:GetCurrentDevmodeBlip()
            if currentBlip then
                --encounterID
                local encounterID, encounterName, description, displayInfo, iconImage = EJ_GetCreatureInfo(1)
                if not encounterID then
                    print("TDT: Error - Make sure to open Encounter Journal and navigate to the boss you want to add!")
                    return
                end
                for i=1,10000 do
                ...
                local data = TDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
                data.isBoss = true
        ...
        button3:SetCallback("OnClick",function()
            TDT:CleanEnemyData(db.currentDungeonIdx)
            local export = tshow(TDT.dungeonEnemies[db.currentDungeonIdx],"TDT.dungeonEnemies[dungeonIndex]")
            TDT.main_frame.ExportFrame:ClearAllPoints()
            TDT.main_frame.ExportFrame:Show()
            TDT.main_frame.ExportFrame:SetPoint("CENTER", TDT.main_frame,"CENTER",0,50)
            TDT.main_frame.ExportFrameEditbox:SetText(export)
    ...
    local function DrawGroup5(container)
        ...
        createSpireButton:SetCallback("OnClick",function()
            ...
            local pois = TDT.mapPOIs[db.currentDungeonIdx][TDT:GetCurrentSubLevel()]
            local posx,posy = 300,-200
            local newWeek = TDT:DeepCopy(week)
            ...
            newWeek = TDT:DeepCopy(week)
            TDT:POI_UpdateAll()
            --add associated NPC to the map
            TDT:AddCloneFromData(tooltipIdxToNpcId[tooltipIndex],newWeek)
        end)
    ...
    local function DrawGroup6(container)
        ...
        resetDbButton:SetCallback("OnClick",function()
            TDT:OpenConfirmationFrame(300, 150, "Reset TDT DB", "Confirm", "Do you want to reset TDT DB?", function()
                TDT:HardReset()
            end, "Cancel", nil)
        end)
    ...
    --hook UpdateMap
    local originalFunc = TDT.UpdateMap
    function TDT:UpdateMap(...)
        originalFunc(...)
        local selectedTab
...
function TDT:AddCloneFromData(npcId, weeks)
    local sublevel= TDT:GetCurrentSubLevel()
    local x,y = 320,-200
    local data
    for _,enemyData in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    print(string.format("TDT: Created clone %s %d at %d,%d",data.name,#data.clones,x,y))
    TDT:UpdateMap()
end
...
function TDT:AddCloneAtCursorPosition()
    if not MouseIsOver(TDTScrollFrame) then return end
    if currentEnemyIdx then
        local data = TDT.dungeonEnemies[db.currentDungeonIdx][currentEnemyIdx]
        local cursorx,cursory = TDT:GetCursorPosition()
        local scale = self:GetScale()
        cursorx = cursorx*(1/scale)
        cursory = cursory*(1/scale)
        tinsert(data.clones,{ x=cursorx, y=cursory, sublevel= TDT:GetCurrentSubLevel(), g=currentCloneGroup, teeming=currentTeeming})
        print(string.format("TDT: Created clone %s %d at %d,%d",data.name,#data.clones,cursorx,cursory))
        TDT:UpdateMap()
    end
end
...
function TDT:AddPatrolWaypointAtCursorPosition()
    if not MouseIsOver(TDTScrollFrame) then return end
    local currentBlip = TDT:GetCurrentDevmodeBlip()
    if currentBlip then
        local data = TDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
        ...
        local patrolBlips = TDT:GetPatrolBlips()
        ...
        print(string.format("TDT: Created Waypoint %d of %s %d at %d,%d",1,data.name,#cloneData.patrol,cursorx,cursory))
        TDT:UpdateMap()
    end
end
