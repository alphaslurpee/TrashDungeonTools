    ...
    for i=29,36 do
        if db.dataCollection[i] then
            for id,spells in pairs(db.dataCollection[i]) do
                local enemies = TDT.dungeonEnemies[i]
...
function TDT:RequestDataCollectionUpdate()
    --temporary lag fix
    if true then return end
    local distribution = self:IsPlayerInGroup()
    if not distribution then return end
    TDTcommsObject:SendCommMessage(self.dataCollectionPrefixes.request, "0", distribution, nil, "ALERT")
end
...
function DC:DistributeData()
    ...
    if not DC.lastDistribution or DC.lastDistribution < GetTime() - 300 then
        ...
        local export = TDT:TableToString(package,false,5)
        TDTcommsObject:SendCommMessage(TDT.dataCollectionPrefixes.distribute, export, distribution, nil, "BULK",nil,nil)
    end
end
...
function DC:InitHealthTrack()
    ...
    for _,enemy in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    f:SetScript("OnEvent", function(self, event, ...)
        if event == "UPDATE_MOUSEOVER_UNIT" then
            ...
            if npcId then
                ...
                for enemyIdx,enemy in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
                    if enemy.id == tonumber(npcId) then
                        if enemy.health ~= npcHealth then
                            ...
                            local enemiesLeft = " "
                            enemiesToScale = {}
                            for k,v in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
...
function TDT:FinishHPTrack()
    ...
    for enemyIdx in pairs(enemiesToScale) do
        local enemy = TDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
