...
function TDT:OpenDataImportDialog()
    TDT:HideAllDialogs()
    local f = TDT.main_frame.dataImportDialog
    f:ClearAllPoints()
...
function TDT:ValidateDataImport(data)
    return true --TODO: y
end

function TDT:CreateDataImportDialog(frame)
    ...
    importButton:SetCallback("OnClick", function()
        local data = TDT:StringToTable(importString, true)
        if TDT:ValidateDataImport(data) then
            db = TDT:GetDB()
...
function TDT:OpenNoDungeonDataWarning()
    local f = TDT.main_frame.noDungeonDataWarning
    f:ClearAllPoints()
    f:SetPoint("CENTER", TDT.main_frame,"CENTER",0,50)
    f:Show()
end

function TDT:CreateNoDungeonDataWarning(frame)
    ...
    f.curseforge:SetText("https://www.curseforge.com/wow/addons/trash-dungeon-tools")
