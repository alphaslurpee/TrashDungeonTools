...
#   journalencounter: contains the encounterID and instanceID for bosses which TDT stores
...
def convert_to_TDT_coord(df):       # Converts mob position to coordinates used for the TDT map. Returns x,y as series
    # TDT INFO FOR SCALE = 1
    # WIDTH = 840
    # HEIGHT = 555
    # HEIGHT IS NEGATIVE
    # 0,0 IS TOP LEFT CORNER
    extent = get_map_extent(df.UiMapID)
    # TDT width and height at scale = 1
    width = 840
    height = 555
    x = float((df.xcoord - extent.xmin) / (extent.xmax - extent.xmin))
    y = float((df.ycoord - extent.ymin) / (extent.ymax - extent.ymin))
...
# Converts combat log coordinates to TDT coordinates
mobHits[["TDTx", "TDTy"]] = mobHits.apply(convert_to_TDT_coord, axis=1)
# Transforms UiMapID into TDT sublevel
...
npc_locale_en = ""
table_output = "TDT.dungeonEnemies[dungeonIndex] = {\n"
...
for unique_npc_index, unique_npc_name in enumerate(mobHits.destName.unique()):
    ...
    for npc_index, npc in enumerate(mobHits[mobHits.destName == unique_npc_name].itertuples()):
        ...
        table_output += f'\t\t\t\t["x"] = {npc.TDTx};\n'
        table_output += f'\t\t\t\t["y"] = {npc.TDTy};\n'
...
table_output += f"\nTDT.dungeonTotalCount[dungeonIndex] = {{normal={total_count},teeming=1000,teemingEnabled=true}}\n\n"
