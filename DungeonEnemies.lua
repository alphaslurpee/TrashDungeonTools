...
function TDT:GetDungeonEnemyBlips()
    return blips
end
...
function TDT:DoFramesOverlap(frameA, frameB, offset)
...
function TDTDungeonEnemyMixin:OnEnter()
    ...
    if self.data.corrupted then
        ...
        if not self.selected then
            local _,active = TDT.poi_framePools:GetPool("VignettePinTemplate"):EnumerateActive()
            for poiFrame,_ in pairs(active) do
                if poiFrame.spireIndex and poiFrame.npcId == self.data.id then
                    ...
                    self.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,self.spireFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                    self.spireFrame.animatedLine = self.animatedLine
                    break
                end
            end
            local connectedDoor = TDT:FindConnectedDoor(self.data.id)
            if connectedDoor then
                self.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,connectedDoor,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
            end
        end
    end
    if not db.devMode then
        self.fontstring_Text1:SetText(TDT:IsCurrentPresetTeeming() and self.data.teemingCount or self.data.count)
        ...
        for _,blip in pairs(blips) do
            if blip.clone.g == self.clone.g then
                blip.fontstring_Text1:SetText(TDT:IsCurrentPresetTeeming() and blip.data.teemingCount or blip.data.count)
...
function TDTDungeonEnemyMixin:OnLeave()
    self:updateSizes(1)
    self:SetFrameLevel(self:GetFrameLevel()-5)
    if db.devMode then
    ...
    TDT:DisplayBlipTooltip(self,false)
    if self.data.corrupted then
        ...
        if not self.selected then
            TDT:HideAnimatedLine(self.animatedLine)
        end
    end
    if not db.devMode then
        if not self.clone.g then
...
local function setUpMouseHandlers(self)
    ...
    self:SetScript("OnDragStart", function()
        local x,y,scale
        preset = TDT:GetCurrentPreset()
        tempPulls = TDT:DeepCopy(preset.value.pulls)
        local targetPull
        local _, _, _, blipX, blipY = self:GetPoint()
        self:SetScript("OnUpdate",function()
            local nx,ny = TDT:GetCursorPosition()
            if x~=nx or y~=ny then
                x,y = nx,ny
                --find closest pull and measure distance
                local pullIdx,centerX,centerY = TDT:FindClosestPull(x,y)
                ...
                if not isClose then
                    targetPull = nil
                    TDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, false, IsControlKeyDown(),tempPulls,nil,true)
                    TDT:DungeonEnemies_UpdateSelected(TDT:GetCurrentPull(),tempPulls)
                elseif pullIdx~=targetPull then
                    targetPull = pullIdx
                    TDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, true, IsControlKeyDown(),tempPulls,pullIdx,true)
                    TDT:DungeonEnemies_UpdateSelected(TDT:GetCurrentPull(),tempPulls)
                end
            end
        end)
...
local function setUpMouseHandlersAwakened(self,clone,scale,riftOffsets)
    ...
    self:SetScript("OnMouseDown",function(self, button)
        if button == "LeftButton" then
            riftOffsets = TDT:GetRiftOffsets()
    ...
    self:SetScript("OnDragStart", function()
        self:StartMoving()
        TDT.draggedBlip = self
        local _,activeDoors = TDT.poi_framePools:GetPool("MapLinkPinTemplate"):EnumerateActive()
        riftOffsets = TDT:GetRiftOffsets()
        self:SetScript("OnUpdate",function()
            for poiFrame,_ in pairs(activeDoors) do
                if TDT:DoFramesOverlap(self,poiFrame,-10) then
                    poiFrame.HighlightTexture:Show()
                else
                    poiFrame.HighlightTexture:Hide()
                end
            ...
            if x ~= self.adjustedX or y~= self.adjustedY then
                local sizex,sizey = TDT:GetDefaultMapPanelSize()
                x = x<=0 and 0 or x>=sizex and sizex or x
                y = y>=0 and 0 or y<=(-1)*sizey and (-1)*sizey or y
                self.adjustedX = x
                self.adjustedY = y
                local connectedDoor = TDT:FindConnectedDoor(self.data.id)
                self.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,connectedDoor or self.spireFrame ,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                riftOffsets[self.data.id] = riftOffsets[self.data.id] or {}
                riftOffsets[self.data.id].x = x
                riftOffsets[self.data.id].y = y
                self:ClearAllPoints()
                self:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
            end
        end)
    end)
    self:SetScript("OnDragStop", function()
        ...
        self:SetScript("OnUpdate",nil)
        self:ClearAllPoints()
        self:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",self.adjustedX*scale,self.adjustedY*scale)
        --dragged ontop of door
        --find doors,check overlap,break,swap sublevel,change poi sublevel
        local _,active = TDT.poi_framePools:GetPool("MapLinkPinTemplate"):EnumerateActive()
        for poiFrame,_ in pairs(active) do
            if TDT:DoFramesOverlap(self,poiFrame,-10) then
                ...
                if shouldAdd then tinsert(c,{ connectionIndex=poiFrame.poi.connectionIndex, source= TDT:GetCurrentSubLevel()+0, target=poiFrame.poi.target}) end
        ...
        if TDT.liveSessionActive and TDT:GetCurrentPreset().uid == TDT.livePresetUID then
            TDT:LiveSession_SendCorruptedPositions(preset.value.riftOffsets)
        end
    end)
end

function TDTDungeonEnemyMixin:OnClick(button, down)

    if button == "LeftButton" then
        if IsShiftKeyDown() then
            local newPullIdx = TDT:GetCurrentPull() + 1
            TDT:PresetsAddPull(newPullIdx)
            TDT:GetCurrentPreset().value.selection = { newPullIdx}
            TDT:ReloadPullButtons()
            TDT:SetSelectionToPull(newPullIdx)
            local _
            TDT:ColorAllPulls(_, newPullIdx)
            --if TDT:GetPullsNum() == 2 then TDT:SetAutomaticColor(1) end
        end
        TDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self,not self.selected,IsControlKeyDown())
        TDT:DungeonEnemies_UpdateSelected(TDT:GetCurrentPull())
        ...
        if self.data.corrupted then
            ...
            self.animatedLine = TDT:ShowAnimatedLine(TDT.main_frame.mapPanelFrame,connectedFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
    ...
    elseif button == "RightButton" then
        if db.devMode then
            if IsAltKeyDown() then
                tremove(TDT.dungeonEnemies[db.currentDungeonIdx][self.enemyIdx].clones,self.cloneIdx)
                self:Hide()
            else
...
function TDT:GetPatrolBlips()
    return patrolPoints
end

function TDTDungeonEnemyMixin:DisplayPatrol(shown)
    local scale = TDT:GetScale()
    ...
    if self.clone.patrol then
        ...
        for patrolIdx,waypoint in ipairs(self.clone.patrol) do
            patrolPoints[patrolIdx] = patrolPoints[patrolIdx] or TDT.main_frame.mapPanelFrame:CreateTexture("TDTDungeonPatrolPoint"..patrolIdx,"BACKGROUND")
            ...
            patrolPoints[patrolIdx]:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",waypoint.x*scale,waypoint.y*scale)
            patrolPoints[patrolIdx].x = waypoint.x
            patrolPoints[patrolIdx].y = waypoint.y
            patrolPoints[patrolIdx]:Show()

            patrolLines[patrolIdx] = patrolLines[patrolIdx] or TDT.main_frame.mapPanelFrame:CreateTexture("TDTDungeonPatrolLine"..patrolIdx,"BACKGROUND")
            patrolLines[patrolIdx]:SetDrawLayer("OVERLAY", 1)
            patrolLines[patrolIdx]:SetTexture("Interface\\AddOns\\TrashDungeonTools\\Textures\\Square_White")
            ...
            if oldWaypointBlip then
                ...
                DrawLine(patrolLines[patrolIdx], TDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
                patrolLines[patrolIdx]:Show()
            else
        ...
        if firstWaypointBlip and oldWaypointBlip then
            ...
            local endPoint, endRelativeTo, endRelativePoint, endX, endY = oldWaypointBlip:GetPoint()
            DrawLine(patrolLines[1], TDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
            patrolLines[1]:Show()
        end
...
local ranOnce
function TDT:DisplayBlipTooltip(blip, shown)
    if not ranOnce then
        --fix elvui skinning
        TDT.tooltip:ClearAllPoints()
        TDT.tooltip:SetPoint("TOPLEFT",UIParent,"BOTTOMRIGHT")
        TDT.tooltip:SetPoint("BOTTOMRIGHT",UIParent,"BOTTOMRIGHT")
        TDT.tooltip:Show()
        TDT.tooltip:Hide()
        ranOnce = true
    end
    ...
    local health = TDT:CalculateEnemyHealth(boss,data.health,db.currentDifficulty,data.ignoreFortified)
    ...
    local text = data.name.." "..occurence..group.."\n"..string.format(L["Level %d %s"],data.level,data.creatureType).."\n".. string.format(L["%s HP"],TDT:FormatEnemyHealth(health)).."\n"
    local count = TDT:IsCurrentPresetTeeming() and data.teemingCount or data.count
    text = text ..L["Forces"]..": ".. TDT:FormatEnemyForces(count)
    local reapingText
    if blip.data.reaping and db.MDI.enabled and preset.mdi.beguiling == 13 then
        local reapingIcon = CreateTextureMarkup(TDT.reapingStatic[tostring(blip.data.reaping)].iconTexture, 32, 32, 16, 16, 0, 1, 0, 1,0,0) or ""
        reapingText = L["Reaping"]..": "..reapingIcon.." ".. TDT.reapingStatic[tostring(blip.data.reaping)].name .. "\n"
    end
    ...
    if db.tooltipInCorner then
        tooltip:SetPoint("BOTTOMRIGHT", TDT.main_frame,"BOTTOMRIGHT",0,0)
        tooltip:SetPoint("TOPLEFT", TDT.main_frame,"BOTTOMRIGHT",-tooltip.mySizes.x,tooltip.mySizes.y)
    else
...
function TDT:GetCurrentDevmodeBlip()
    for blipIdx,blip in pairs(blips) do
        if blip.devSelected then
            return blip
        end
...
local function blipDevModeSetup(blip)
    ...
    blip:SetScript("OnMouseDown",function()
        ...
        x = x*(1/scale)
        y = y*(1/scale)
        local nx = TDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x
        local ny = TDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y
        xOffset = x-nx
        yOffset = y-ny
    end)
    ...
    blip:SetScript("OnDragStop", function()
        ...
        blip:ClearAllPoints()
        blip:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
        TDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x = x
        TDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y = y
    end)
    local groupColors = {
        [1] = {1,0,0,1},
        [2] = {0,1,0,1},
    ...
    blip:SetScript("OnMouseWheel", function(self, delta)
        ...
        if not blip.clone.g then
            local maxGroup = 0
            for _,data in pairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
...
function TDTDungeonEnemyMixin:SetUp(data,clone)
    local scale = TDT:GetScale()
    self:ClearAllPoints()
    self:SetPoint("CENTER", TDT.main_frame.mapPanelTile1,"TOPLEFT",clone.x*scale,clone.y*scale)
    self.normalScale = data.scale*(data.isBoss and 1.7 or 1)*(TDT.scaleMultiplier[db.currentDungeonIdx] or 1)*scale
    ...
    if not data.corrupted then
        for k,v in pairs(blips) do
            ...
            if ((clone.x-v.clone.x)^2+(clone.y-v.clone.y)^2<81) and TDT:DoFramesOverlap(self, v,5) then raise = max(raise,v:GetFrameLevel()+1) end
        end
    end
    ...
    if data.corrupted then
        ...
        local riftOffsets = TDT:GetRiftOffsets()
...
function TDT:DungeonEnemies_IsAnyBlipMoving()
    local isAnyMoving
    for blipIdx,blip in pairs(blips) do
...
function TDT:DungeonEnemies_HideAllBlips()
    TDT.dungeonEnemies_framePools:ReleaseAll()
end

function TDT:DungeonEnemies_UpdateEnemies()
    TDT.dungeonEnemies_framePools:ReleaseAll()
    twipe(blips)
    if not db then db = TDT:GetDB() end
    local enemies = TDT.dungeonEnemies[db.currentDungeonIdx]
        ...
        for cloneIdx,clone in pairs(data["clones"]) do
            ...
            if clone.sublevel == currentSublevel or (not clone.sublevel) then
                ...
                if not (data.corrupted and riftOffsets and riftOffsets[data.id] and riftOffsets[data.id].sublevel) then
                    local blip = TDT.dungeonEnemies_framePools:Acquire("TDTDungeonEnemyTemplate")
...
function TDT:DungeonEnemies_CreateFramePools()
    db = self:GetDB()
    TDT.dungeonEnemies_framePools = TDT.dungeonEnemies_framePools or CreateFramePoolCollection()
    TDT.dungeonEnemies_framePools:CreatePool("Button", TDT.main_frame.mapPanelFrame, "TDTDungeonEnemyTemplate");
end

function TDT:FindPullOfBlip(blip)
...
function TDT:GetBlip(enemyIdx, cloneIdx, preset)
    for blipIdx,blip in pairs(blips) do
...
function TDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(blip, add, ignoreGrouped,pulls,pull,ignoreUpdates)
...
function TDT:DungeonEnemies_UpdateBlipColors(pull, r, g, b,pulls)
...
function TDT:DungeonEnemies_UpdateSelected(pull,pulls)
    preset = TDT:GetCurrentPreset()
    pulls = pulls or preset.value.pulls
    --deselect all
    for _,blip in pairs(blips) do
    ...
    for pullIdx,p in pairs(pulls) do
        local r,g,b = TDT:DungeonEnemies_GetPullColor(pullIdx)
        for enemyIdx,clones in pairs(p) do
            if tonumber(enemyIdx) then
                for _,cloneIdx in pairs(clones) do
                    for _,blip in pairs(blips) do
    ...
    TDT:DrawAllHulls(pulls)
end

---DungeonEnemies_SetPullColor
---Sets a custom color for a pull
function TDT:DungeonEnemies_SetPullColor(pull, r, g, b)
    preset = TDT:GetCurrentPreset()
    preset.value.pulls[pull]["color"] = TDT:RGBToHex(r,g,b)
end

---DungeonEnemies_GetPullColor
---Returns the custom color for a pull
function TDT:DungeonEnemies_GetPullColor(pull,pulls)
    pulls = pulls or preset.value.pulls
    local r,g,b = TDT:HexToRGB(pulls[pull]["color"])
    if not r then
        r,g,b = TDT:HexToRGB(db.defaultColor)
        TDT:DungeonEnemies_SetPullColor(pull,r,g,b)
    end
    return r,g,b
end
...
function TDT:DungeonEnemies_UpdateTeeming()
    preset = TDT:GetCurrentPreset()
...
function TDT:DungeonEnemies_UpdateSeasonalAffix()
...
function TDT:DungeonEnemies_UpdateBlacktoothEvent()
    local week
...
function TDT:DungeonEnemies_UpdateBoralusFaction(faction)
    preset = TDT:GetCurrentPreset()
    local teeming = TDT:IsPresetTeeming(preset)
    for _,blip in pairs(blips) do
...
function TDT:DungeonEnemies_UpdateReaping()
    for _,blip in pairs(blips) do
...
function TDT:DungeonEnemies_UpdateInfested(week)
    week = preset.week%3
    if week == 0 then week = 3 end
    for _,blip in pairs(blips) do
...
function TDT:DungeonEnemies_UpdateInspiring(week)
    week = week or preset.week
    local isInspiring = TDT:IsWeekInspiring(week)
    for _,blip in pairs(blips) do
...
function TDT:DungeonEnemies_UpdateFreeholdCrew(crewIdx)
    ...
    local crew = TDT.freeholdCrews[crewIdx]
    for _,blip in pairs(blips) do
        ...
        if enemy.id == poi.npcId then
            ...
            for cloneIdx,clone in pairs(enemy.clones) do
                if clone.week[week] then
                    return TDT:IsCloneInPulls(enemyIdx,cloneIdx)
                end
            end
        end
    end
...
function TDT:IsCloneInPulls(enemyIdx, cloneIdx)
    local pulls = TDT:GetCurrentPreset().value.pulls
    local numClones = 0
...
function TDT:GetEnemyForces(npcId)
...
function TDT:GetNPCNameById(npcId)
    local data = TDT.dungeonEnemies[db.currentDungeonIdx]
    if data then
...
function TDT:UpdateDungeonData(dungeonData)

    local function printDungeonName(shouldPrint,dungeonIdx)
        if shouldPrint then
            print("-----", TDT:GetDungeonName(dungeonIdx))
        end
        return false
    end
    ...
    for dungeonIdx,newData in pairs(dungeonData) do
        ...
        local totalCount = TDT.dungeonTotalCount[dungeonIdx]
        ...
        local enemyData = TDT.dungeonEnemies[dungeonIdx]
        if enemyData then
...
--]
function TDT:ExportNPCIdsWithoutDisplayIds()
    local output = "dungeons = [\n"
    for idx = 15, TDT:GetNumDungeons() do
        local shouldAddDungeonText = true
        local enemyData = TDT.dungeonEnemies[idx]
        if enemyData then
            for _,enemy in pairs(enemyData) do
                if not enemy.displayId then
                    if shouldAddDungeonText then
                        output = output.."Dungeon(name='".. TDT:GetDungeonName(idx).."', idx=".. idx..", npcIds=["
    ...
    TDT.main_frame.ExportFrameEditbox:SetLabel("NPC ids without displayId")
end
...
function TDT:CleanEnemyData(dungeonIdx)
    local enemies = TDT.dungeonEnemies[dungeonIdx]
