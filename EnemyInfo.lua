...
local function MakeEnemeyInfoFrame()
    ...
    f.frame:SetAllPoints(TDTScrollFrame)

    local originalHide = TDT.main_frame.Hide
    function TDT.main_frame:Hide(...)
        f.frame:Hide()
    ...
    local function DrawGroup1(container)
        ...
        TDT:FixAceGUIShowHide(model,modelContainer.frame,true)
        ...
        sendSpellsButton:SetCallback("OnClick",function()
            ...
            SendChatMessage(string.format(L["TDT: Spells for %s:"],enemyName),distribution)
...
local lastEnemyIdx
function TDT:UpdateEnemyInfoFrame(enemyIdx)
    if not enemyIdx then enemyIdx = lastEnemyIdx end
    lastEnemyIdx = enemyIdx
    if not enemyIdx then return end
    local data = TDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
    local f = TDT.EnemyInfoFrame
    ...
    local enemies = {}
    for mobIdx,edata in ipairs(TDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    TDT:UpdateEnemyInfoData(enemyIdx)
    ...
    if data.spells then
        for spellId,spellData in pairs(data.spells) do
            if not spellBlacklist[spellId] then
                local spellButton = AceGUI:Create("TDTSpellButton")
...
function TDT:UpdateEnemyInfoData(enemyIdx)
    local f = TDT.EnemyInfoFrame
    if not enemyIdx then enemyIdx = lastEnemyIdx end
    if not enemyIdx then return end
    local data = TDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
    ...
    local healthText = TDT:FormatEnemyHealth(health)
    ...
    local fortifiedTyrannical = TDT:IsCurrentPresetFortified() and L["Fortified"] or L["Tyrannical"]
    f.enemyDataContainer.healthEditBox:SetLabel(string.format(L["Enemy Info NPC Health"],level,fortifiedTyrannical))
end

function TDT:ShowEnemyInfoFrame(blip)
    db = TDT:GetDB()
    TDT.EnemyInfoFrame = TDT.EnemyInfoFrame or MakeEnemeyInfoFrame()
    TDT:UpdateEnemyInfoFrame(blip.enemyIdx)
    TDT.EnemyInfoFrame:Show()
end
